...
def get_output(res_1, res_2, res_3, res_4, res_5):
    """Функция для вывода данных о работе функций"""
    ...
    print(res_4)
    print("Название алгоритма:")
    print(res_5)
    print()


def main():
    """Функция, реализующая основную логику программы"""
    matrix = get_matrix_from_keyboard()
    print("Введённая матрица:")
    print_matrix(matrix)
    print()

    # flag - решение существует
    if matrix_of_coefficients_is_square(matrix):
        matrix_of_coefficients = get_matrix_of_coefficients(matrix)
        numpy_det = linalg.det(matrix_of_coefficients)
        if abs(numpy_det) <= DELTA:
            my_own_det = det(matrix_of_coefficients)
        ...
        print("Матрица коэффициентов не является квадратной")
        return

    if flag:
        res_matrix, inversed_matrix, matrix_of_solutions = jacobis_method_of_simple_iterations(matrix)
        number_of_conditionality = infinite_norm(res_matrix) * infinite_norm(inversed_matrix)
        get_output(res_matrix, inversed_matrix, matrix_of_solutions, number_of_conditionality, "Метод простых итераций Якоби")
        if isinstance(number_of_conditionality, complex):
            number_of_conditionality = default_sqrt(number_of_conditionality.imag ** 2 + number_of_conditionality.real ** 2)
        if number_of_conditionality < 100:
            return
        elif number_of_conditionality > 100:
            res_matrix, inversed_matrix, matrix_of_solutions = jordan_gauss_direct_algorithm(matrix)
            number_of_conditionality = infinite_norm(res_matrix) * infinite_norm(inversed_matrix)
            get_output(res_matrix, inversed_matrix, matrix_of_solutions, number_of_conditionality, "Прямой алгоритм Гаусса-Жордана")
            if isinstance(number_of_conditionality, complex):
                number_of_conditionality = default_sqrt(number_of_conditionality.imag ** 2 + number_of_conditionality.real ** 2)
            if number_of_conditionality < 100:
                return
            elif number_of_conditionality > 100:
                res_matrix, inversed_matrix, matrix_of_solutions = direct_gauss_Jordan_algorithm_with_calculations_of_correct_fractions(matrix)
                number_of_conditionality = infinite_norm(res_matrix) * infinite_norm(inversed_matrix)
                get_output(res_matrix, inversed_matrix, matrix_of_solutions, number_of_conditionality, "Прямой алгоритм Гаусса-Жордана с вычислениями правильных дробей")
                return
    else:
        print("Матрица вырожденная, единого решения не существует")
        return
